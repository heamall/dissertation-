...
# Sample data - Al2O3 (wt%) and Cr (ppm)
data <- data.frame(
  ...
  Cr_ppm = c(204.0, 339.0, 165.0, 95.0, 246.0,
             116.0, 281.0, 119.0, 252.0, 128.0, 255.0, 1204.0)
)
...
ggplot() +
  ...
  geom_point(data = data,
             aes(x = Al2O3, y = Cr_ppm, colour = group),
             size = 4, shape = 16) +
  # Repelled sample labels
  geom_text_repel(data = data,
                  aes(x = Al2O3, y = Cr_ppm, label = sample, colour = group),
                  size = 3.2,
                  box.padding = 0.4,
                  point.padding = 0.3,
                  segment.size = 0.3,
  ...
  labs(
    ...
    y       = "Cr (ppm)",
    title   = expression(Al[2]*O[3]~"vs Cr"),
